...
## ⑰ Trade review·edge 반영 — 결정 불필요 명세 보강 (2026-06-22)

- **trd-01 #1-5/#1-6**: 오픈 0건(사용 증거금 0) 시 Margin Level="—"(N/A) · Free Margin=Equity 전액 · Open P&L·Margin=$0.00 · Open=0.
- **trd-05 #4**: 최소 이체 금액(기본 $1.00, 정책 확정 시 조정) 미만 시 비활성+인라인 에러.
- **trd-05 #5**: Security Number 미설정 시 설정 유도(front-set-05)→복귀 분기 · 중복 제출 방지(버튼 잠금·idempotency).
- **trd-03-01 #2**: 활성화 진행 중 이탈→재진입 시 중복 계좌 생성 없이 상태 복원.
- **trd-07 #2-1**: 세션 토큰 만료 시 자동 재로그인 1회→실패 시 Error+Retry.
- **trd-04 #3-1 / 인덱스**: 용어 통일 — "Funding↔Trading" → "Cash↔Trading account".
- **trd-02 #3**: 스토어 딥링크 실패·미지원 OS·미설치 시 웹 다운로드 폴백.
- (보류) Trade account ↔ Trading account 명명 통일 / 거래계좌 Equity 더미 정합 / 다계좌 #1 지표 기준 / Transfer 회수 Stop-out 정책 = 결정 필요.

## ⑱ 노션 IA 전체 싱크 — 정본 89행 재생성 (2026-06-22)

- 화면설계서(4파일) 권위 인벤토리 = **89화면** 추출(중복/구버전 없음).
- 노션 일괄 조회(쿼리)는 플랜 제약으로 불가 → 기존 'ROGer IA 구조도 보기'(낡음·고아행 다수)는 **보존**하고, **새 정본 DB 'ROGer IA 구조도 (화면설계서 기준 · 2026-06-22)'를 IA 구조도(회원) 하위에 신설**해 89행 일괄 생성.
  - URL: https://app.notion.com/p/a15475d8476f41f3906268faa2e709b8
  - 컬럼: GNB·1~4 Depth·화면 ID·Name·우선순위·기능 및 상세설명.
  - GNB 분포: Portfolio 12 · Trade 9 · Earning 6 · Account 24 · More 38.
- 부모 IA 페이지 상단에 '최신 정본' 콜아웃 추가.
- (남음) **유저플로우 페이지**는 다음 턴에 화면설계서 기준으로 직접 반영 예정.

## ⑲ Trade 결정 8건 반영 (2026-06-22)

디스크립션 미정 항목 + review/edge 결정 확정:
- **기준 통화**: USD 단일 고정(드롭다운 없음). (trd-03 #4-2)
- **연동 해제**: 포지션/잔액 있으면 차단 → 청산·회수 유도 후 처리(완료 시 해제). (trd-04 #5-2)
- **ROG 입금 주소**: 계정당 고정 주소 1개 재사용(회전 없음). (acc-07 #4)
- **카드 입장 코드**: 초대 대상에게 이메일·푸시·이벤트로 발송된 코드 입력. (card-10)
- **거래계좌 Equity 더미 정합**: trd-01·trd-04 Equity=$1,500.00로 통일(pos-00 Broker와 일치). 종속 지표 재설정 — Margin $300·Margin Level 500%·Free Margin $1,200·Open P&L +$58.02·Today P&L +$24.18.
- **용어 통일**: "Trading account" → **"Trade account"** (전 13곳).
- **다계좌 지표**: 계좌 2개 이상 시 trd-01 #1 지표는 대표(주) 계좌 기준 표시, #2 행에서 전환.
- **Transfer 회수**: 거래계좌→Cash 회수로 Margin Level 위험 구간 진입 시 경고(차단 안 함, 사용자 판단). (trd-05 #4)
- (보류) MT5 WebView·PG·정산사·Provider 브랜드·거래소 로고 = 외부 계약/에셋 의존. Leverage 상한 = 계약·규제 확정.
